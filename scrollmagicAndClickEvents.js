    ...
    scene.addIndicators({name: "3rd Child Activation"});
    ...
    var playerResize = cld.videoPlayer('cloudinaryResizeVideo', {
        autoplay: false,
        loop: false,
        muted: true,
        controls: false,
    ...
    if (!cursorXray || !cursorResize || !offsetXray || !offsetResize) {
        ...
        function updateCursorVisibility(e) {
            ...
            lastCursorY >= offsetXrayRect.top && lastCursorY <= offsetXrayRect.bottom);
            const isOutResize = !(lastCursorX >= offsetResizeRect.left && lastCursorX <= offsetResizeRect.right &&
            lastCursorY >= offsetResizeRect.top && lastCursorY <= offsetResizeRect.bottom);
        ...
        let reverseVideoUrl = 'https://res.cloudinary.com/davkwryyf/video/upload/e_reverse/v1710533238/resize-prep-constantFramerate30-WEB_gcumxs.mp4';
        playerResize.source('resize-prep-constantFramerate30-WEB_gcumxs',{
            sourceTypes: ['mp4'],
            transformation: [{ quality: 'auto:best' }],
            src: reverseVideoUrl
        });
        function playVideoForDuration(videoElement, duration) {
            videoElement.play();
            setTimeout(() => {
                videoElement.pause();
            }, duration);
        }
        setVideoSource(playerResize, reverseVideoUrl);
            cursorResize.addEventListener('click', function() {
                ...
                //playerResize.currentTime(1/30);
                if (addIconOpacityClamp > 0.5) {
                    console.log(`Incrementing frame.`);
                    console.log(`Before change: ${playerResize.currentTime()}`);
                    playVideoForDuration(videoElement, 200);
                    console.log(`After change: ${playerResize.currentTime()}`);
                    ...
                    if(previousFrame >= 0) { 
                            ...
                            playVideoForDuration(videoElement, 200);
                            console.log(`After change: ${playerResize.currentTime()}`);
                            console.log(`Decrementing frame.`);
                        }   else {
